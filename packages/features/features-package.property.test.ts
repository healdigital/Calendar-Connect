 ...
 * Feature: ee-removal
 */
describe("Property Tests: Features Package EE Removal", () => {
  /**
   ...
   *
   * For any TypeScript or JavaScript file in the features package,
   * the file should not contain import statements matching the patterns
   * `@/ee`, `@calcom/features/ee`, or `@calcom/ee`
   */
  it("Property 1: No EE imports in features package", async () => {
    // Get all TypeScript and JavaScript files in the features package
    const featuresPackagePath = path.join(process.cwd(), "packages/features");
    const files = await glob("**/*.{ts,tsx,js,jsx}", {
      cwd: featuresPackagePath,
